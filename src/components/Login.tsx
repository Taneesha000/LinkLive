  ...
  return (
    <div className="bg-gray-900 rounded-xl p-6 h-full flex items-center justify-center">
      <form onSubmit={submit} className="bg-gray-800/50 p-6 rounded-lg w-full max-w-sm space-y-4">
        <div className="text-white text-xl font-bold text-center">Login</div>
        {!isAuthConfigured() && (
        ...
        <button disabled={loading} className="w-full bg-cyan-600 hover:bg-cyan-500 disabled:opacity-60 rounded py-2 text-white">{loading ? 'Signing in...' : 'Sign in'}</button>
      </form>
    </div>
  );
}
